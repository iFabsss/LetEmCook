...
def is_code_expired(request):
    ...
    created_at = created_at.replace(tzinfo=std_timezone.utc)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not title or not ingredients:
                return JsonResponse({'error': 'Title and ingredients are required'}, status=400)
...
def generate_tags(request):
    if request.method == 'POST':
        ...
        try:
            ...
            prompt = f"Do not return any explanation, just the string that contains the tags that are comma-separated. Generate SEO-friendly product tags (comma-separated). Generate tags for a recipe called {title} with the following ingredients: {ingredients}. The description of the recipe is: {description}. Return only comma-separated tags, nothing else."
